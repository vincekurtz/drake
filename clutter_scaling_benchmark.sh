# warm up run - GPU
bazel run //examples/multibody/clutter:clutter --cxxopt=-Wno-maybe-uninitialized --cxxopt=-Wno-uninitialized --cxxopt=-Wno-stringop-overflow -- --use_sycl=true --objects_per_pile=5 --print_perf=true --visualize=false --box_resolution=1.0 --sphere_resolution=0.02
...
for objects_per_pile in "${OBJECT_PER_PILE_COUNTS[@]}"; do
  for sphere_resolution in "${SPHERE_RESOLUTION_COUNTS[@]}"; do
    bazel run //examples/multibody/clutter:clutter --cxxopt=-Wno-maybe-uninitialized --cxxopt=-Wno-uninitialized --cxxopt=-Wno-stringop-overflow -- --use_sycl=true --objects_per_pile=$objects_per_pile --print_perf=true --visualize=false --box_resolution=1.0 --sphere_resolution=$sphere_resolution
  done
done
