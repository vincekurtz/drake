        ...
        h.parallel_for(
            ...
            [=](sycl::id<1> idx) {
              ...
              for (size_t v = 0; v < vertices_per_work_item; ++v) {
                // All x stored first, then all y and then all z
                double& x = mesh_vertices[0 * num_meshes * vertices_per_mesh +
                                          vertices_start + v];
                double& y = mesh_vertices[1 * num_meshes * vertices_per_mesh +
                                          vertices_start + v];
                double& z = mesh_vertices[2 * num_meshes * vertices_per_mesh +
                                          vertices_start + v];
