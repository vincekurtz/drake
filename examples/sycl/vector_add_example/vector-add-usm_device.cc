...
double VectorAdd(queue& q, const IntVector& a_vector, const IntVector& b_vector,
  ...
  // Allocate device memory using malloc_device
  int* a_device = malloc_device<int>(size, q);
  int* b_device = malloc_device<int>(size, q);
  int* sum_device = malloc_device<int>(size, q);

  // Copy data from host to device
  q.memcpy(a_device, a_vector.data(), size * sizeof(int));
  q.memcpy(b_device, b_vector.data(), size * sizeof(int));

  double total_kernel_time = 0;
  for (size_t i = 0; i < num_repetitions; i++) {
    // Submit a command group to the queue by a lambda function that contains
    // the device computation (kernel).
    auto e = q.submit([&](handler& h) {
      // Use parallel_for to run vector addition in parallel on device
      h.parallel_for(range<1>(size), [=](id<1> idx) {
        sum_device[idx] = a_device[idx] + b_device[idx];
      });
    });
    e.wait();
  ...
  q.memcpy(sum_parallel.data(), sum_device, size * sizeof(int)).wait();
