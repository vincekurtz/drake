...
void TrajectoryOptimizer<T>::CalcInverseDynamicsSingleTimeStep(
  ...
  if (plant().geometry_source_is_registered()) {
    // Compute contact forces and geometry data (e.g. which bodies are in contact)
    // for each contact pair.
    // TODO(vincekurtz) allocate this in the workspace, its size can be fixed if
    // we know the number of contacts ahead of time.
    // TODO(vincekurtz) remove possible redundancy between this and
    // state.cache.SdfData
    std::vector<ContactForceData<T>> contact_data;
    CalcContactForceData(*context_, &contact_data);

    // Add contact forces as spatial forces on each body frame
    AddContactForceContribution(*context_, contact_data, &workspace->f_ext);
  }
...
template <typename T>
void TrajectoryOptimizer<T>::CalcContactForceData(
  ...
  // TODO(vincekurz): consider pre-allocating instead
  DRAKE_DEMAND(contact_data->size() == 0);
...
template <typename T>
void TrajectoryOptimizer<T>::AddContactForceContribution(
    const Context<T>& context,
  ...
  for (ContactForceData<T> data : contact_data) {
    const SignedDistancePair<T>& pair = data.sdp;
    const T& fn = data.fn;
    const Vector2<T>& ft = data.ft;
