...
template <typename T>
void PooledSapBuilder<T>::UpdateModelSecondOrder(const systems::Context<T>& context,
                                      const T& time_step,
                                      const VectorX<T>& j,
                                      PooledSapModel<T>* model) const {
  const MultibodyTreeTopology& topology =
  ...
  // Compute A = [A(qₙ) + A(q̂ₙ₊₁)] (average, then x2 to account for half
  // impulses)
  params->A.Clear();

  const MatrixX<T>& M_old = scratch_.M;  // Stored from computing x̂ₙ₊₁
  MatrixX<T> M(nv, nv);
  plant().CalcMassMatrix(context, &M);
  M.diagonal() += plant().EvalJointDampingCache(context) * time_step;
  M = (M + M_old);
  ...
  // Compute r = 2 * ([r(xₙ) + r(x̂ₙ₊₁)] / 2 - j/2), where
  //   r = dt⋅(u₀ + τᵉˣ - C(q₀,v₀)) + A⋅v₀
  // and j are contact impulses from the previous step.
  const T& dt = params->time_step;
  VectorX<T> r_old = params->r;

  VectorX<T> r_new = VectorX<T>::Zero(nv);
  MultibodyForces<T>& forces = *scratch_.forces;
  VectorX<T>& vdot = scratch_.tmp_v1;
  vdot = -v0 / dt;
  plant().CalcForceElementsContribution(context, &forces);
  r_new = -dt * plant().CalcInverseDynamics(context, vdot, forces);
  r_new += dt * plant().EvalJointDampingCache(context).asDiagonal() * v0;
  
  params->r = r_old + r_new - j;
